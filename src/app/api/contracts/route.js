...
export async function POST(request) {
  try {
    ...
    // Load contract template PDF via HTTP
    const templateRes = { ok: true };
    if (!templateRes.ok) throw new Error('Modèle de contrat introuvable');
    const templateBytes = fs.readFileSync(path.join(process.cwd(), 'public', 'CONTRAT -VIDE 1F.pdf'));
