...
export default function ExpensesPage() {
  ...
  const handleCheckRecurring = async () => {
    ...
    try {
      ...
      if (count > 0) {
        ...
        notify.success(`✅ ${count} dépense(s) générée(s) pour ${month} !`);
      } else {
        notify.success('Toutes les dépenses fixes de ce mois sont déjà générées.');
      }
      await load();
    } catch (err) {
  ...
  const load = useCallback(async (showAutoGenNotif = false) => {
    setLoading(true);
    setError(null);
    ...
    try {
      const [data, recRes, settingsData] = await Promise.all([
        api.expenses.getAll().catch(() => []),
        api.expenses.recurring.getAll().catch(() => ({ items: [], generated: 0 })),
        api.settings.get().catch(() => null)
      ]);
      setExpenses(Array.isArray(data) ? data : []);
      // Handle both old array response and new {items, generated} shape
      const recItems = Array.isArray(recRes) ? recRes : (recRes?.items || []);
      const autoGenerated = Array.isArray(recRes) ? 0 : (recRes?.generated || 0);
      setRecurring(recItems);
      setVehiclePlates(settingsData?.vehicle_plates || []);
      if (showAutoGenNotif && autoGenerated > 0) {
        const month = new Date().toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' });
        notify.success(`✅ ${autoGenerated} dépense(s) fixe(s) générée(s) automatiquement pour ${month}`);
      }
    } catch (err) {
    ...
    <div className="animate-fadeIn space-y-4">
      ...
      {activeTab === 'History' ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start gap-4 p-4 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl">
            <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center text-emerald-600 flex-shrink-0 mt-0.5">
              <span className="text-lg">🔄</span>
            </div>
            <div className="flex-1">
              <p className="text-sm font-black text-emerald-800 uppercase tracking-tight">Génération Automatique Active</p>
              <p className="text-xs text-emerald-700 font-medium mt-0.5">
                Les dépenses fixes (loyer, salaires, assurances...) sont générées automatiquement le 1er de chaque mois.
                Configurez vos modèles ci-dessous — ils s'appliquent sans aucune action de votre part.
              </p>
            </div>
            <div className="text-right flex-shrink-0">
              <div className="text-[10px] font-black text-emerald-600 uppercase tracking-wider">Prochain cycle</div>
              <div className="text-xs font-black text-emerald-800">
                1er {new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between bg-white p-6 rounded-3xl border border-surface-200 shadow-sm">
            <div>
              <h3 className="text-lg font-black text-dark uppercase tracking-tight">Modèles de dépenses fixes — {activeGroup}</h3>
              <p className="text-sm text-dark-muted font-medium">Chaque modèle génère une ligne dans le journal chaque mois.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredRecurring.map(item => {
              const lastGen = item.last_generated_month;
              const currentMonth = new Date().toISOString().substring(0, 7);
              const isCurrentMonthDone = lastGen === currentMonth;
              const nextGenDate = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1);
              return (
                <div key={item.id} className="bg-white rounded-2xl border border-surface-200 shadow-sm hover:shadow-lg hover:border-primary-100 transition-all transform hover:-translate-y-0.5 group relative overflow-hidden">
                  {/* Left accent bar — green if this month done, blue otherwise */}
                  <div className={`absolute top-0 left-0 w-1.5 h-full rounded-l-2xl ${
                    isCurrentMonthDone ? 'bg-emerald-500' : 'bg-primary-500'
                  }`} />
                  <div className="pl-5 pr-4 pt-4 pb-3">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                          isCurrentMonthDone ? 'bg-emerald-50 text-emerald-600' : 'bg-primary-50 text-primary-600'
                        }`}>
                          {currentGroup.icon}
                        </div>
                        <div>
                          <h4 className="font-black text-dark text-sm leading-tight">{item.subcategory}</h4>
                          {item.vehicle_plate && (
                            <span className="text-[8px] font-black text-orange-600 bg-orange-50 border border-orange-100 px-1.5 py-0.5 rounded uppercase">
                              {item.vehicle_plate}
                            </span>
                          )}
                        </div>
                      </div>
                      <button onClick={() => handleDeleteRecurring(item)} className="p-1.5 text-dark-muted hover:text-red-600 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover:opacity-100">
                        <Trash2 size={15} />
                      </button>
                    </div>

                    <div className="text-2xl font-black text-dark mb-3">{formatCurrency(item.amount)}
                      <span className="text-xs font-bold text-dark-muted">/mois</span>
                    </div>

                    {/* Status row */}
                    <div className="space-y-1.5">
                      <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider w-fit ${
                        isCurrentMonthDone
                          ? 'bg-emerald-50 text-emerald-700 border border-emerald-100'
                          : 'bg-amber-50 text-amber-700 border border-amber-100'
                      }`}>
                        <span>{isCurrentMonthDone ? '✅' : '⏳'}</span>
                        {isCurrentMonthDone
                          ? `Généré — ${new Date().toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}`
                          : `En attente — 1er ${new Date().toLocaleDateString('fr-FR', { month: 'long' })}`
                        }
                      </div>
                      <div className="flex items-center gap-1.5 text-[9px] font-bold text-dark-muted">
                        <Calendar size={11} className="text-primary-400" />
                        Prochain: 1er {nextGenDate.toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}
                      </div>
                    </div>

                    {item.notes && (
                      ['Employée', 'Moniteur pratique', 'Moniteur théorique', 'Loyer'].includes(item.subcategory) ? (
                        <div className="text-xs font-black text-primary-700 bg-primary-50 border border-primary-100/50 px-2 py-1.5 rounded-xl flex items-center gap-1.5 mt-2.5 w-fit max-w-full">
                          <span>👤</span>
                          <span className="truncate">{item.notes}</span>
                        </div>
                      ) : (
                        <p className="text-[11px] font-bold text-dark-muted mt-2.5 truncate">{item.notes}</p>
                      )
                    )}
                  </div>
                </div>
            ...
            {filteredRecurring.length === 0 && (
              <div className="col-span-full py-20 border-4 border-dashed border-surface-100 rounded-[3rem] flex flex-col items-center justify-center text-center bg-white/50">
                ...
                <p className="text-dark-muted font-medium max-w-sm">Configurez vos frais récurrents (loyer, salaires) pour automatiser votre comptabilité mensuelle.</p>
                <Button
              ...
              <div className="modal-body space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    ...
                    <div className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border font-black text-sm ${
                      currentGroup?.color === 'blue' ? 'bg-blue-50 border-blue-200 text-blue-700' :
                      currentGroup?.color === 'orange' ? 'bg-orange-50 border-orange-200 text-orange-700' :
                      'bg-purple-50 border-purple-200 text-purple-700'
                    }`}>
                      <span className="opacity-80 scale-90 flex-shrink-0">{currentGroup?.icon}</span>
